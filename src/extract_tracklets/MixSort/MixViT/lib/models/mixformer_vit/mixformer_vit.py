...
class VisionTransformer(timm.models.vision_transformer.VisionTransformer):
    ...
    def __init__(self, img_size_s=256, img_size_t=128, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768,
        ...
        self.target_emb = nn.Parameter(torch.zeros(1, 1, 1), requires_grad=True) ## tesla服务器
        self.sear_emb = nn.Parameter(torch.zeros(1, 1, 1), requires_grad=True) ## tesla服务器
    ...
    def forward(self, x_t, x_s):
        ...
        for blk in self.blocks:
            ...
            if i == 9:    ## tesla服务器
                flag_emb = torch.cat([self.target_emb.repeat(1, H_t*W_t, 1), self.sear_emb.repeat(1, H_s*W_s, 1)], dim=1) ## tesla服务器
                x = x + flag_emb ## tesla服务器
            i += 1 ## tesla服务器
