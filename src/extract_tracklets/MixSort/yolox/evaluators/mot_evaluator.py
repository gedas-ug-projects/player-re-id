...
class MOTEvaluator:
    ...
    def evaluate_mixsort(
        ...
        assert result_path.endswith(
            ".txt"
        ), f"Result path must end with .txt, but got {result_path}"
        settings = {
        ...
        for cur_iter, (origin_imgs, imgs, _, info_imgs, ids) in tqdm(
            enumerate(progress_bar), total=len(progress_bar)
        ):
            # ONE FORWARD PASS
            with torch.no_grad():

                # need to stack these up
                frame_id = info_imgs[2].item()
                video_id = info_imgs[3].item()
                img_file_name = info_imgs[4]
                ...
                if ((cur_iter) % batch_size == 0) and cur_iter > 0:
                    ...
                    if video_name not in video_names:
                        video_names[video_id] = video_name
                    logger.debug(f"video_names: {video_names}")
                ...
                if (cur_iter) % batch_size == 0:
                    for o, ii, id in zip(outputs_post_proccessed, info_images_batch, ids_batch):
                        ...
                        # output_results = self.convert_to_coco_format(outputs, info_images_batch, ids_batch)
